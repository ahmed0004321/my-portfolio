          ...
          <div className="hidden lg:flex items-center gap-8 xl:gap-12">
            {navLinks.map(link => (
              <Link key={link.path} to={link.path}
                className={`text-sm font-medium tracking-wide line-animate transition-colors duration-300 ${location.pathname === link.path ? 'text-primary' : 'text-foreground hover:text-primary'
                  }`}
              >{link.label}</Link>
            ))}
          </div>
        ...
        <div ref={menuItemsRef} className="h-full flex flex-col items-center justify-center container-custom">
          ...
          <div className="absolute bottom-8 sm:bottom-12 left-0 right-0 container-custom flex flex-col sm:flex-row justify-between items-center sm:items-end gap-4">
            <div className="text-muted-foreground text-xs sm:text-sm text-center sm:text-left">
              ...
              <p className="text-primary">ahmed0004321@gmail.com</p>
            </div>
            <div className="flex gap-4 sm:gap-6 text-xs sm:text-sm">
              {[
                { name: 'LinkedIn', url: 'https://www.linkedin.com/in/oasif-ahmed-rikto-30610b354/' },
                { name: 'GitHub', url: 'https://github.com/ahmed0004321' },
              ].map(s => (
                <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors">{s.name}</a>
              ))}
            </div>
          </div>
        </div>
